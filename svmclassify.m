...
%   $Revision: 1.1.12.7 $  $Date: 2007/06/04 21:05:46 $
...
% check inputs
if nargin < 2
    error('Bioinfo:svmclassify:NotEnoughInputArguments',nargchk(2,Inf,nargin))
end
        ...
        elseif length(k)>1
            ...
            switch(k)
                case 1 % plotflag
                    plotflag = opttf(pval);
                    if isempty(plotflag)
                        error('Bioinfo:svmclassify:ShowPlotNotLogical','%s must be a logical value, true or false.',...
                            upper(char(okargs(k))));
                    end
    ...
    if plotflag
        ...
        if isempty(svmStruct.FigureHandles)
            ...
            try
                ...
                [hAxis,hClassLines] = svmplotdata(sampleOrig,classified,hAxis); %#ok
